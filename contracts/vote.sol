...
contract VotingSystem {
    ...
    function registerAsVoter() external {
        ...
        roles[msg.sender] = Role.Voter;
        emit Registered(msg.sender, Role.Voter);
    }

    function registerAsCandidate() external {
        require(roles[msg.sender] == Role.None, "Already registered");
        roles[msg.sender] = Role.Candidate;
        emit Registered(msg.sender, Role.Candidate);
    }

    function registerAsAdmin() external {
        require(msg.sender == owner, "Only owner can self-register as admin");
        roles[msg.sender] = Role.Admin;
        emit Registered(msg.sender, Role.Admin);
    }

    // Admin can also assign roles manually
    function assignRole(address _user, Role _role) external onlyAdmin {
        require(_role != Role.None, "Invalid role");
        roles[_user] = _role;
        emit Registered(_user, _role);
    }
